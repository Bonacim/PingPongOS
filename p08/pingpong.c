...
#define TICKS 20
#define PREEMP_OFF()	int save_state = currentTask->allowPreemp; currentTask->allowPreemp = 0;	
#define PREEMP_ON()		currentTask->allowPreemp = save_state;

task_t mainTask, dispatcherTask, *currentTask;
queue_t *ready_queue, *suspend_queue;
...
int task_create (task_t *task, void (*start_func)(void *), void *arg)
{
    ...
    if (stack)
    {
        ...
        if (task != &dispatcherTask)
        {
            ...
            task->wqueue = (task_t*)ready_queue;
            task->status = pronta;
            task_setprio(task, 0);
            userTasks++;
        }
...
void task_exit (int exitCode)
{
    ...
    while (currentTask->joined != NULL) 
    {
        task_t* aux = currentTask->joined;
        if (currentTask->joined == currentTask->joined->next)
            currentTask->joined = NULL;
        else
            currentTask->joined = currentTask->joined->next;
        task_resume(aux);
    }
...
task_t *scheduler()
{
	...
	while(aux != NULL && aux->next != (task_t*)ready_queue)
	{
		aux = aux->next;
...
void task_suspend (task_t *task, task_t **queue)
{
    if (queue)
    {
        ...
        // if (aux == (task_t*)ready_queue)
        //     ready_queue = ready_queue->next;
        queue_remove(&ready_queue, (queue_t*)aux);
...
{    
    ...
    if (task != NULL && task != currentTask) 
    {
        ...
        int found = 0;
        int i;
        for(i = queue_size((queue_t*)ready_queue); i > 0; i--)
        {
            if (aux == task) 
            {
                found = 1;
		...
		{
			#ifdef DEBUG
			//printf("task_join: A tarefa %d esta esperando a tarefa %d\n",task_id(),task->id);
			#endif
	
			task_suspend(currentTask,&(task->joined));
	
			task_switch(&dispatcherTask);						//Entrega o processador e o dispatcher suspende a tarefa
