...
int sem_create (semaphore_t *s, int value)
{
	...
	task_class save = current_task->t_class;
    current_task->t_class = System;
	...
	s->next = NULL;
	s->prev = NULL;
	queue_append(&semaphore_queue,(queue_t*)s);
...
int sem_destroy (semaphore_t *s)
{
	...
	queue_remove (&semaphore_queue,(queue_t*)s);
        ...
        for (q_size = queue_size ((queue_t*)barrier_queue); q_size > 0; q_size--)
        {
            if (b_aux == b)
            {
                ...
                for (tasks_quant = queue_size ((queue_t*)b->task_queue); tasks_quant > 0; tasks_quant--)
                {
                    t_aux->barriExit = -1;
                    t_aux = t_aux->next;
                    task_resume (t_aux->prev);
                }

                queue_remove ((queue_t**)&barrier_queue, (queue_t*)b);
